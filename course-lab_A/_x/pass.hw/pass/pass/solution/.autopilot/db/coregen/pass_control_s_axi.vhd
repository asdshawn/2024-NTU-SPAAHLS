...
-- 0x24 : reserved
-- 0x28 : Data signal of numInputs
--        bit 31~0 - numInputs[31:0] (Read/Write)
-- 0x2c : reserved
-- 0x30 : Data signal of processDelay
--        bit 31~0 - processDelay[31:0] (Read/Write)
-- 0x34 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of pass_control_s_axi is
    ...
    constant ADDR_NUMINPUTS_DATA_0    : INTEGER := 16#28#;
    constant ADDR_NUMINPUTS_CTRL      : INTEGER := 16#2c#;
    constant ADDR_PROCESSDELAY_DATA_0 : INTEGER := 16#30#;
    constant ADDR_PROCESSDELAY_CTRL   : INTEGER := 16#34#;
    ...
    signal int_numInputs       : UNSIGNED(31 downto 0) := (others => '0');
    signal int_processDelay    : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUMINPUTS_DATA_0 =>
                        rdata_data <= RESIZE(int_numInputs(31 downto 0), 32);
                    when ADDR_PROCESSDELAY_DATA_0 =>
                        rdata_data <= RESIZE(int_processDelay(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    numInputs            <= STD_LOGIC_VECTOR(int_numInputs);
    processDelay         <= STD_LOGIC_VECTOR(int_processDelay);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUMINPUTS_DATA_0) then
                    int_numInputs(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_numInputs(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_PROCESSDELAY_DATA_0) then
                    int_processDelay(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_processDelay(31 downto 0));
                end if;
            end if;
        end if;
    end process;
