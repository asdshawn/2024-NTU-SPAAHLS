...
#(parameter
    ...
    output wire [31:0]                   numInputs,
    output wire [31:0]                   processDelay,
...
localparam
    ...
    ADDR_NUMINPUTS_DATA_0    = 6'h28,
    ADDR_NUMINPUTS_CTRL      = 6'h2c,
    ADDR_PROCESSDELAY_DATA_0 = 6'h30,
    ADDR_PROCESSDELAY_CTRL   = 6'h34,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_numInputs = 'b0;
    reg  [31:0]                   int_processDelay = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NUMINPUTS_DATA_0: begin
                    rdata <= int_numInputs[31:0];
                end
                ADDR_PROCESSDELAY_DATA_0: begin
                    rdata <= int_processDelay[31:0];
                end
            endcase
        end
    end
...
assign numInputs     = int_numInputs;
assign processDelay  = int_processDelay;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_numInputs[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUMINPUTS_DATA_0)
            int_numInputs[31:0] <= (WDATA[31:0] & wmask) | (int_numInputs[31:0] & ~wmask);
    end
end

// int_processDelay[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_processDelay[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PROCESSDELAY_DATA_0)
            int_processDelay[31:0] <= (WDATA[31:0] & wmask) | (int_processDelay[31:0] & ~wmask);
    end
end
